...
**ベストプラクティスを意識させない**: エージェント開発者がプロンプトにルールを書き連ねなくても、sidekick を通すだけでエビデンス検証・段階的開示・Intent Gate が適用される。正しいやり方がデフォルトになる設計。

**エージェントの進化に任せ、ハーネスに徹する**: LLM は急速に賢くなる。推論能力やコンテキスト理解の向上はエージェント側に委ね、sidekick は構造的な制約と検証——ハーネス——だけを担う。エージェントの能力を代替するのではなく、その能力が正しく発揮される枠組みを提供する。

## このプロダクトの価値

sidekick が解決する問題は3つに集約される。

**ハルシネーションの抑制**: エージェントがタスクを委譲する際、inputs に本文だけでなくエビデンス（citation: 出典 + 抜粋）を添付する。ワーカーは原典を辿って裏取りできる。sidekick はエビデンスの存在検証を自動化できる構造を持つ。LLM の「それっぽい要約」ではなく、検証可能な参照に基づいてタスクが流れる。

**段階的開示のアーキテクチャ化**: ワーカーはタスクに必要な情報だけを受け取る。メインエージェントの会話履歴、hooks、設定は漏洩しない。プロセス分離により、ワーカーのコンテキストに入るものを sidekick が制御する。これは UI の段階的開示と同じ原則——必要な情報を、必要なときに、必要な粒度で渡す——をエージェント間通信に適用したものである。

**ワークフローの宣言的実行**: タスクの種類、必要な入力、後続ステップを Markdown の frontmatter で宣言する。sidekick はこの定義に従ってチェーン実行、output 解決、バリデーションを行う。ワークフローの追加にコード変更は不要。
